...
local instructions = {
    [0x0] = function(x, y, n, nn, nnn)
        if nn == 0xE0 then
            ...
            if debug_c then print("(op 0) cls") end
        elseif nn == 0xEE then
            cpu.sp = cpu.sp - 1
            cpu.pc = cpu.stack[cpu.sp]
            if debug_c then print("(op 0) return from subroutine") end
        end
    end,
    [0x1] = function(x, y, n, nn, nnn)
        cpu.pc = nnn
        if debug_c then print("(op 1) jp to line " .. string.format(nnn)) end
    end,
    [0x2] = function(x, y, n, nn, nnn)
        cpu.stack[cpu.sp] = cpu.pc
        cpu.sp = cpu.sp + 1
        cpu.pc = nnn
        if debug_c then print("(op 2) jump to subroutine at line " .. string.format(nnn)) end
    end,
    [0x3] = function(x, y, n, nn, nnn)
        if debug_c then print("(op 3) check") end
        if cpu.reg[x] == nn then
            cpu.pc = cpu.pc + 2
            if debug_c then print("(op 3) skipped cmd") end
        end
    end,
    [0x4] = function(x, y, n, nn, nnn)
        if debug_c then print("(op 4) check") end
        if cpu.reg[x] ~= nn then
            cpu.pc = cpu.pc + 2
            if debug_c then print("(op 4) skipped cmd") end
        end
    end,
    [0x5] = function(x, y, n, nn, nnn)
        if debug_c then print("(op 5) check") end
        if cpu.reg[x] == cpu.reg[y] then
            cpu.pc = cpu.pc + 2
            if debug_c then print("(op 5) skipped cmd") end
        end
    end,
    [0x6] = function(x, y, n, nn, nnn)
        cpu.reg[x] = nn
        if debug_c then print("(op 6) set V" .. string.format("%x", x):upper() .. " " .. string.format(nn)) end
    end,
    [0x7] = function(x, y, n, nn, nnn)
        cpu.reg[x] = bit.band(cpu.reg[x] + nn, 255)
        if debug_c then print("(op 7) add to V" .. string.format("%x", x):upper() .. " " .. string.format(nn)) end
        --print(cpu.reg[x])
    end,
    [0x8] = function(x, y, n, nn, nnn)
        if n == 0x0 then
            cpu.reg[x] = cpu.reg[y]
            if debug_c then print("(op 8) V" .. string.format("%x", x):upper() .. " equals V" .. string.format("%x", y):upper()) end
        elseif n == 0x1 then
            cpu.reg[x] = bit.bor(cpu.reg[x], cpu.reg[y])
            if debug_c then print("(op 8) V" .. string.format("%x", x) .. " equals V" .. string.format("%x", x):upper() .. " OR V" .. string.format("%x", y):upper()) end
        elseif n == 0x2 then
            cpu.reg[x] = bit.band(cpu.reg[x], cpu.reg[y])
            if debug_c then print("(op 8) VX equals VX AND VY") end
        elseif n == 0x3 then
            cpu.reg[x] = bit.bxor(cpu.reg[x], cpu.reg[y])
            if debug_c then print("(op 8) VX equals VX XOR VY") end
        elseif n == 0x4 then
            local sum = cpu.reg[x] + cpu.reg[y]
            if sum > 255 then
                cpu.reg[0xF] = 1
                if debug_c then print("(op 8) VX equals VX plus VY [carry]") end
            else
                cpu.reg[0xF] = 0
                if debug_c then print("(op 8) VX equals VX plus VY [no carry]") end
            end
            cpu.reg[x] = bit.band(sum, 255)
        elseif n == 0x5 then
            if cpu.reg[x] >= cpu.reg[y] then
                cpu.reg[0xF] = 1
                if debug_c then print("(op 8) VX equals VX minus VY [no borrow]") end
            else
                cpu.reg[0xF] = 0
                if debug_c then print("(op 8) VX equals VX minus VY [borrow]") end
            end
        ...
        elseif n == 0x6 then
            if cpu.s_mode == 0 then
                ...
                if debug_c then print("(op 8) VX equals VY shifted right [original shift]") end
            else
                cpu.reg[0xF] = bit.band(cpu.reg[x], 1)
                cpu.reg[x] = bit.rshift(cpu.reg[x], 1)
                if debug_c then print("(op 8) VX equals VX shifted right [modern shift]") end
            end
        elseif n == 0x7 then
            if cpu.reg[y] >= cpu.reg[x] then
                cpu.reg[0xF] = 1
                if debug_c then print("(op 8) VX equals VY minus VX [no borrow]") end
            else
                cpu.reg[0xF] = 0
                if debug_c then print("(op 8) VX equals VY minus VX [borrow]") end
            end
        ...
        elseif n == 0xE then
            if cpu.s_mode == 0 then
                ...
                if debug_c then print("(op 8) VX equals VY shifted left [original shift]") end
            else
                cpu.reg[0xF] = bit.rshift(bit.band(cpu.reg[x], 0x80), 7)
                cpu.reg[x] = bit.band(bit.lshift(cpu.reg[x], 1), 255)
                if debug_c then print("(op 8) VX equals VX shifted left [modern shift]") end
            end
        end
    end,
    [0x9] = function(x, y, n, nn, nnn)
        if debug_c then print("(op 9) check") end
        if cpu.reg[x] ~= cpu.reg[y] then
            cpu.pc = cpu.pc + 2
            if debug_c then print("(op 9) skipped cmd") end
        end
    end,
    [0xA] = function(x, y, n, nn, nnn)
        cpu.idx = nnn
        if debug_c then print("(op A) register i equals " .. string.format(nnn)) end
    end,
    [0xB] = function(x, y, n, nn, nnn)
        cpu.pc = nnn + cpu.reg[0x0]
        if debug_c then print("(op B) PC equals NNN plus V0") end
    end,
    [0xC] = function(x, y, n, nn, nnn)
        local rand = math.random(0, 255)
        cpu.reg[x] = bit.band(rand, nn)
        if debug_c then print("(op C) VX equals random 0/255 with mask of " .. string.format(nn)) end
    end,
    [0xD] = function(x, y, n, nn, nnn)
        local stx = cpu.reg[x] % 64
        local sty = cpu.reg[y] % 32
        cpu.reg[0xF] = 0
        if debug_c then print("(op D) draw sprite at " .. string.format(cpu.reg[x]) .. ", " .. string.format(cpu.reg[y]) .. " at memory address " .. string.format(cpu.idx) .. " with height " .. string.format(nn)) end
    ...
    [0xE] = function(x, y, n, nn, nnn)
        ...
        if nn == 0x9E then
            if debug_c then print("(op E) check if key " .. string.format(cpu.reg[x]) .. " is pressed") end
            if cpu.keys[kidx] then
                cpu.pc = cpu.pc + 2
            end
        elseif nn == 0xA1 then
            if debug_c then print("(op E) check if key " .. string.format(cpu.reg[x]) .. " is not pressed") end
    ...
    [0xF] = function(x, y, n, nn, nnn)
        if nn == 0x07 then
            ...
            if debug_c then print("(op F) set V" .. string.format(x) .. "to delay timer") end
        elseif nn == 0x0A then
            cpu.pause = true
            cpu.key_tar = x
            if debug_c then print("(op F) check if key pressed") end
        elseif nn == 0x15 then
            if debug_c then print("(op F) delay timer equals V" .. string.format("%x", x):upper()) end
            cpu.del_timer = cpu.reg[x]
        elseif nn == 0x18 then
            if debug_c then print("(op F) sound timer equals V" .. string.format("%x", x):upper()) end
            cpu.aud_timer = cpu.reg[x]
        elseif nn == 0x1E then
            if debug_c then print("(op F) add register i with V" .. string.format("%x", x):upper()) end
            cpu.idx = bit.band(cpu.idx + cpu.reg[x], 0x0FFF)
        elseif nn == 0x29 then
            if debug_c then print("(op F) set register i to address of character " .. string.format(cpu.reg[x]) .. " in memory") end
...
function cpu.init()
    ...
    for line in file:lines() do
        ...
        if line:find("debug") then
            local val = string.sub(line, 9, #line)
            if val == "true" then debug = true else debug = false end
        end
        if line:find("debug_c") then
            local val = string.sub(line, 11, #line)
            if val == "true" then debug_c = true else debug_c = false end
        end
    end
...
local function drawDebug()
    ...
    love.graphics.print("PC: " .. string.format(cpu.pc), 0, 320)
    love.graphics.print("INDEX: " .. string.format(cpu.idx), 0, 340)
    if cpu.stack[0] then love.graphics.print("TOP OF STACK: " .. string.format(cpu.stack[0]), 0, 360) else love.graphics.print("TOP OF STACK: NULL", 0, 360) end
    love.graphics.print("STACK POINTER: " .. string.format(cpu.sp), 0, 380)
    love.graphics.print("FLAG: " .. string.format(cpu.reg[0xF]), 0, 400)
    love.graphics.print(string.format("OPCODE: %x", c), 0, 420)
    love.graphics.print("DELAY: " .. string.format(cpu.del_timer), 0, 440)
    love.graphics.print("SOUND: " .. string.format(cpu.aud_timer), 0, 460)
    for i = 0, #cpu.reg-1 do
        love.graphics.print(string.format("V%x: %i", i, cpu.reg[i]), 200, (i*20)+320)
    end
    for i = 0, #cpu.keys do
        if cpu.keys[i] then love.graphics.print(string.format("KEY %x: %s", i, cpu.keys[i]), 400, (i*20)+320) else love.graphics.print(string.format("KEY %x: false", i), 400, (i*20)+320) end
    end
end
